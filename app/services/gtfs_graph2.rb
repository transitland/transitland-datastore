class GTFSGraph2
  ...
  def find_or_initialize_stop(gtfs_entity)
    # Check cache
    ...
    if tl_entity
      ...
      tl_entity.parent_stop = find_or_initialize_stop(@gtfs.stop(gtfs_entity.parent_station))
      log "NEW: #{tl_entity}"
    else
      tl_entity = Stop.new
      log "NEW: #{tl_entity}"
    ...
    tl_entity.timezone = gtfs_entity.stop_timezone
    tl_entity.tags = {
  ...
  def find_or_initialize_route(gtfs_entity)
    ...
    tl_entity.wheelchair_accessible = 0
    tl_entity.bikes_allowed = 0
  ...
  def find_or_initialize_rsp(gtfs_entity, tl_stops)
    ...
    # Update
    add_eiff(tl_entity, gtfs_entity)
    tl_entity.stop_pattern = stop_pattern
      ...
      gtfs_agency.routes.each do |gtfs_route|
        ...
        gtfs_route.trips.each do |gtfs_trip|
          ...
          tl_stops = gtfs_trip.stops.map do |gtfs_stop|
            ...
            find_or_initialize_stop(gtfs_stop)
          end
          tl_route.serves |= tl_stops

          # Create RSP
          tl_rsp = find_or_initialize_rsp(gtfs_trip, tl_stops)
  ...
  def create_changeset
    # Update OnestopIDs
    @entity_tl.each { |gtfs_entity,tl_entity| tl_entity.onestop_id = tl_entity.generate_onestop_id }
    @entity_tl.each { |gtfs_entity,tl_entity|
      (tl_entity.serves = tl_entity.serves.map(&:onestop_id)) if tl_entity.instance_of?(Route)
      (tl_entity.serves = tl_entity.serves.map(&:onestop_id)) if tl_entity.instance_of?(Operator)
      (tl_entity.traversed_by = tl_entity.traversed_by.onestop_id) if tl_entity.instance_of?(RouteStopPattern)
    }
    # Changeset
