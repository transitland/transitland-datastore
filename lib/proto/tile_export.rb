...
class GraphID
  ...
  attr_accessor :value
  def initialize(value: nil, level: 0, tile: 0, index: 0)
    @value = value || (self.class.make_id(level: level, tile: tile, index: index))
  end

  def self.ll_to_tile(tile_level, lat, lon)
  ...
  def get_ll
    ...
    return [(tile / width).to_i * size - 90, (tile % width) * size - 180]
  end

  def self.make_id(level: nil, tile: nil, index: nil)
...
# puts "value: #{graphid.value} bin: #{graphid.value.to_s(2)}"
# puts "level: #{graphid.level} tile: #{graphid.tile} index: #{graphid.index}"
#
# graphid = GraphID.new(level: 2, tile: GraphID.ll_to_tile(2, lat, lon))
# puts "lat: #{lat} lon: #{lon} get_ll: #{graphid.get_ll}"
# puts "value: #{graphid.value} bin: #{graphid.value.to_s(2)}"
# puts "level: #{graphid.level} tile: #{graphid.tile} index: #{graphid.index}"
