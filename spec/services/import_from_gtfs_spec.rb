describe ImportFromGtfs do
  ...
  context 'import' do
    ...
    it "will create records (Stop and Identifier's) for each stop" do
      @vta_gtfs_import.import
      expect(Stop.count).to eq 3859
      expect(Identifier.count).to eq 7718
      expect(Identifier.first.tags['gtfs_source']).to eq 'vta_gtfs.zip'
    end
