describe Geometry do
  ...
  context 'distance calculation' do
    ...
    it 'handles case of stop slightly out of order with previous, and identical matching segments. readjusts distances.' do
      ...
      expect(RouteStopPattern.first.stop_distances).to match_array_within([35.9,295.3,747.8,1061.6,1652.3,1946.8,4168.1,4616.7,4994.4,5547.6,5573.1,6063.4,6282.9,6524.0,6682.7,6775.9,6961.1,7505.6,8912.2,9572.1,10265.7,11055.5,11547.6,11822.3,12294.7,12653.7,13071.7,13371.8,13862.3,14025.0,14184.7,15050.8,15923.9,16247.5,16636.5], 1.0)
      expect(Issue.where(issue_type: 'distance_calculation_inaccurate').count).to eq 0
    end
    ...
    it 'handles alleghany stop distances' do
      ...
      expect(Geometry::TLDistances.new(RouteStopPattern.first).calculate_distances).to match_array([0.0, 1564.3, 2948.4, 7916.3, 15691.7, 21963.3, 28515.8, 34874.6, 35537.6, a_value_within(2.0).of(37510.9), 38152.8, 39011.8, 40017.6, 41943.4, 51008.5, 57260.7, 64464.1, 70759.1])
    end
